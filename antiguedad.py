...
appa.layout = html.Div([dcc.Location(id="url"), sidebar, content])
    ...
    [
        html.Div(
            [
                ...
                dcc.Dropdown(
                supervisores,
                supervisores,multi=True,placeholder="SUPERVISOR...",searchable=True,id='dSUPERVISOR'
                ),
                dcc.Dropdown(
                responsables,
...
@appa.callback([Output("table","data"),Output("table","columns"),Output("table","page_size"),Output("graph1","figure")], [Input('dORIGEN','value'), Input('dSTATUS','value'), Input('dSUPERVISOR','value'), Input('dRESPONSABLE','value')])
def upTable(v_origen,v_status,v_supervisor,v_responsable):
...
@appa.callback(
    Output("output-graph", "children"), Input("table", "active_cell"),[State('dORIGEN','value'), State('dSTATUS','value'), State('dSUPERVISOR','value'), State('dRESPONSABLE','value')]
)
def cell_clicked(active_cell,v_origen,v_status,v_supervisor,v_responsable):
